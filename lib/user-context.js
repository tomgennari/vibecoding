...
/** Placeholder hasta que /api/resolve-random-house actualice el perfil (no mostrar House real equivocado). */
const RANDOM_HOUSE_PLACEHOLDER = {
  id: 'random',
  name: 'Asignando…',
  color: '#64748b',
  image: HOUSES[0].image,
};
...
async function fetchUserData(uid) {
  const [profileRes, unlocksCountRes, sessionsRes, housePointsRes] = await Promise.all([
    supabase.from('profiles').select('first_name, last_name, house, user_type, created_at, unlock_credits, has_all_access, all_access_at').eq('id', uid).single().then((r) => ({ data: r.data, error: r.error })).catch(() => ({ data: null })),
...
export function UserProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: profLite } = await supabase.from('profiles').select('house').eq('id', uid).maybeSingle();
    if (profLite?.house === 'random' && token) {
      await fetch('/api/resolve-random-house', {
        method: 'POST',
  ...
  /** Actualiza campos del perfil en memoria (p. ej. unlock_credits tras desbloquear con crédito). */
